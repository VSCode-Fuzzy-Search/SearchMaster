...
export class LeftPanelWebview implements WebviewViewProvider {
	...
	private activateMessageListener() {
		this._view.webview.onDidReceiveMessage((message: { action: any; data: { message: string; input: string; }; }) => {
			switch (message.action){
				case 'SHOW_WARNING_LOG':
					window.showWarningMessage(message.data.message);
					window.showWarningMessage(message.data.input);
					break;
				case 'ENTER_SEARCH':
					window.showInformationMessage(message.data.message);
					break;
				default:
					break;
			}
